...
export default function Cart() {
    ...
    const loadProductsForOrder = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/api/v1/order/user/${id}/products`);
    ...
    return (
        <div className="container">
            ...
            <button  style={{ backgroundColor: 'yellow', color: 'black', fontSize: '16px', padding: '10px', marginTop: '20px', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
              Place Order
            </button>
        </div>
    );
